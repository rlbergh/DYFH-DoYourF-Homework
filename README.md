# DYFH-DoYourF-Homework
## App to track time spent on homework assignments

### [Change log](https://github.com/rlbergh/DYFH-DoYourF-Homework/blob/main/change_log.md)
### [Instructions](https://github.com/rlbergh/DYFH-DoYourF-Homework/blob/main/Instructions.txt) (how to package this with the .ico and .py files to an exe application)

### Screenshot:

![application screenshot showing to do list](https://github.com/rlbergh/DYFH-DoYourF-Homework/blob/main/DYFH-UI.png)
